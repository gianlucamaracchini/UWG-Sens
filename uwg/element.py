...
class Element(object):
    ...
    def __init__(self, albedo, emissivity, layer_thickness_lst, material_lst,
                 vegcoverage, t_init, horizontal, name, to_UCM=0, HBR=False, HBRalbedo=0.6):
        ...
        self._name = name
        self.HBR = HBR #HBR
        self.to_UCM = to_UCM #HBR
        self.HBRalbedo = HBRalbedo #HBR
    ...
    def to_dict(self):
        ...
        base['name'] = self.name
        base['HBR'] = self.HBR #HBR
        base['to_UCM'] = self.to_UCM #HBR
        return base
    ...
    def SurfFlux(self, forc, parameter, simTime, humRef, tempRef, windRef, boundCond,
        ...
        if self.horizontal:
            ...
            if self.HBR == False:
                ...
                self.sens = self.vegSen + self.aeroCond * (self.layerTemp[0] - tempRef)
                self.flux = self.solAbs + self.infra - \
                    self.lat - self.sens  # [W m-2]
                self.HBRvent_flux = 0
            else:
                # Sensible & net heat flux for HBR 
                self.HBRvent_flux = self.solAbs - self.HBRalbedo * self.solRec #HBR
                
                self.sens = self.vegSen + self.aeroCond * (self.layerTemp[0] - tempRef) + \
                        self.HBRvent_flux #Example of HBR heat waste
