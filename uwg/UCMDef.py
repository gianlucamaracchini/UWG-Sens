...
class UCMDef(object):
    ...
    Args:
        ...
        road: Road element class (mowved from BEM).
    ...
    def UCModel(self, BEM, T_ubl, forc, parameter):
        ...
        self.roofTemp = 0.
        self.wallTemp = 0.
        self.indoorTemp = 0. #HBR
        self.roofFlux = 0. #HBR
        self.coolConsump0 = 0. #HBR only first building
        ...
        #Q = self.roadArea * self.sensAnthrop + (self.roofArea + self.roadArea) * self.treeSensHeat #HBR 
        # modificato per avere il calore immesso dal traffico proporzionale alla superficie stradale
        ...
        for j in range(len(BEM)):
            ...
            T_roof = BEM[j].roof.layerTemp[0] #HBR
            h_conv_roof = BEM[j].roof.aeroCond #HBR
            R_glazing = building.glazing_ratio
            A_wall = (1. - R_glazing) * self.facArea
            A_roof = self.roofArea #HBR
            ...
            H1 = H1 + BEM[j].frac*(
                ...
                BEM[j].roof.to_UCM * T_roof * self.roofArea * h_conv_roof + # HBR roof conv
            ...
            H2 = H2 + BEM[j].frac * (
                ...
                BEM[j].roof.to_UCM * A_roof * h_conv_roof + # HBR roof conv
            ...
            Q = Q + BEM[j].frac * (
                ...
                BEM[j].roof.to_UCM * A_roof * (BEM[j].roof.vegSen + BEM[j].roof.HBRvent_flux) + # HBR roof waste + vegsen
            ...
            self.Q_ubl = (self.Q_ubl + BEM[j].frac * self.bldDensity *
                          (BEM[j].roof.sens * (1. - BEM[j].roof.to_UCM) #HBR
                           + BEM[j].building.sensWaste *                         
                          (1. - self.h_mix)))  # Changed by Jiachen Mao in March 2017
            
            self.indoorTemp = self.indoorTemp + BEM[j].frac * T_indoor #HBR
        ...
        for j in range(len(BEM)):
            ...
            self.roofFlux = self.roofFlux + BEM[j].frac * self.bldDensity * BEM[j].roof.flux #HBR Energy flux through the roof
        ...
        self.coolConsump0 = BEM[0].building.coolConsump #HBR W per m^2 of floor area (W m-2) only for the first building
        self.sensCoolDemand0 = BEM[0].building.sensCoolDemand #HBR W per m^2 of floor area (W m-2) only for the first building
        self.dehumDemand0 = BEM[0].building.dehumDemand_ #HBR
        self.sensWaste0 = BEM[0].building.sensWaste / BEM[0].building.nFloor #HBR
        # ------------------------------------------------------------------------------
        # Sensible Heat
